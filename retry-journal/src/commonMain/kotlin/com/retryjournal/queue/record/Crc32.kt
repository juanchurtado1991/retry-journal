...
import com.retryjournal.queue.record.Crc32.TABLE
import com.retryjournal.queue.record.Crc32.finalize
import com.retryjournal.queue.record.Crc32.update
 ...
 * [DiskQueue][com.retryjournal.queue.disk.DiskQueue] needs: every record written to disk is
 ...
 * match, and [DiskQueue][com.retryjournal.queue.disk.DiskQueue] treats that as "this is where
 * the valid data ends," truncating the corrupt tail instead of crashing or reading garbage back
 * as if it were a real request. See [DiskQueue][com.retryjournal.queue.disk.DiskQueue]'s own
 ...
 * way to get the same corruption check on every target `:retry-journal` compiles for, without pulling
 ...
 * (multi-KB+); at the record sizes `:retry-journal` actually checksums — a small metadata struct plus
 ...
 * for small records.
 *
 * That "small records" assumption is no longer purely hypothetical: [RetryJournalOfflineQueuePlugin][com.retryjournal.client.RetryJournalOfflineQueuePlugin]
 * captures file/image upload bodies too (up to [MAX_RECORD_FIELD_SIZE][com.retryjournal.queue.disk.DiskQueueConstants.MAX_RECORD_FIELD_SIZE], 64 MiB),
