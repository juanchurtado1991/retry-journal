 ...
 * [com.retryjournal.RetryJournal] for why the *payload* serializer is never this library's
 * concern), so it is never re-encoded to reach the queue and never Base64-inflated to fit inside
 * this record.
 *
 * `@GhostSerialization` on purpose: this is the one piece of `:retry-journal` where Ghost's
