...
2. **Tap “Upload a file”** (or “Send 5 JSON requests”). The app tries to send, fails, and **retry-journal writes the full request to disk**.
...
That’s the product: **nothing lost**, sync when you’re back online. In production you’d call `runtime.flush()` / `runtime.flushWhenOnline()` from WorkManager or a connectivity callback — the sample also **auto-flushes** when the health poll reports online via `RetryJournalRuntime`.
...
Implementation reference: [`RetryJournalSampleApplication.kt`](composeApp/src/androidMain/kotlin/com/retryjournal/sample/app/RetryJournalSampleApplication.kt) / [`RetryJournalBackgroundSetup.kt`](composeApp/src/iosMain/kotlin/com/retryjournal/sample/app/RetryJournalBackgroundSetup.kt) (`:retry-worker` wiring) and [`SyncSetup.kt`](composeApp/src/commonMain/kotlin/com/retryjournal/sample/app/SyncSetup.kt) (`RetryJournalRuntime` wiring).

---

## RetryJournalRuntime in this sample

[`SyncSetup`](composeApp/src/commonMain/kotlin/com/retryjournal/sample/app/SyncSetup.kt) builds:

1. `DiskQueue`, `DeadLetterQueue`, `RetryJournalEngine`, live + replay `HttpClient`s (manual wiring)
2. `RetryJournalRuntime.createForEngine(syncEngine, replayClient, …, connectivity = connectivityState)`
...
| `:retry-worker` background task | `runtime.flushWhenOnline()` |
...
./gradlew :retry-sample:composeApp:installDebug
```
...
| Module | Role |
|---|---|
| `:retry-journal` | The library (Maven artifact) |
| `:retry-worker` | Out-of-the-box background scheduler (Maven artifact) — WorkManager / `BGTaskScheduler` |
| `:retry-sample:shared` | Shared ViewModel + sync wiring |
| `:retry-sample:composeApp` | Compose UI + desktop/Android/iOS shells |
| `:retry-sample:server` | Chaos HTTP server for demos |
