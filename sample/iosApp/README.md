...
`iosArm64`/`iosSimulatorArm64` targets of `:sample:composeApp` on this host; Gradle just skips
...
1. In Xcode (or Android Studio's KMP wizard), create a new **Multiplatform App** / **iOS App**
   project named `iosApp` inside `sample/`, or use the standard Compose Multiplatform project
...
3. Add the Kotlin framework: in the Xcode project's build settings / Package Dependencies, link
   against the `ComposeApp` framework produced by `:sample:composeApp:iosArm64` /
   `:sample:composeApp:iosSimulatorArm64` (a "Kotlin Framework" build phase or
   `embedAndSignAppleFrameworkForXcode` Gradle task, same as any KMP+Compose project).
4. Set `AppConstants.SERVER_HOST` (in `sample/composeApp/src/commonMain/.../AppConstants.kt`) to
...
- `ContentView.swift` — the standard `UIViewControllerRepresentable` wrapper around
  `MainViewController()` (from `sample/composeApp/src/iosMain/.../MainViewController.kt`).
...
1. Run `:sample:server:run` (chaos server) and the app side by side.
